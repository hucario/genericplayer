...
class SampleStation extends Station {
	...
	play = () => {
		this.parent.playStation(this); // I would pass a token if this were real but it's not so who cares
...
class SampleExtension extends Extension {
	...
	constructor() {
		...
		setInterval(() => {
			if (!this.currentlyPlaying.playing  || !this.currentlyPlaying.song) {
				return;
			}
	...
	setVolume = (a) => {
		this.currentlyPlaying.volume = Math.min(Math.max(a, 0), 100);
	}
